...
type Props = {
  /**
   * "tab" -> rendered as a tab screen, X navigates to Home tab.
   * "modal" -> rendered as pushed route from shortcut, X pops back.
   */
  mode: "tab" | "modal";
};

export default function AiDetectScreen({ mode }: Props) {
  ...
  const handleClose = () => {
    if (mode === "tab") {
      // Always go to Home tab explicitly
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <View style={styles.header}>
        <Pressable
          ...
          hitSlop={10}
        >
          <Ionicons name={mode === "tab" ? "home-outline" : "close"} size={26} color={colors.onSurface} />
        </Pressable>
